...
## ⚠️ BREAKING CHANGE - January 2026

**The backend now enforces LIVE Stripe only. All test mode logic has been removed for production safety.**

See [PRODUCTION_STRIPE_CONFIG.md](./PRODUCTION_STRIPE_CONFIG.md) for detailed documentation.
...
```bash
# REQUIRED: Stripe Live Secret Key
export STRIPE_LIVE_SECRET_KEY=sk_live_xxxxxxxxxxxxxxxxxxxxx

# RECOMMENDED: Webhook Secret
export STRIPE_LIVE_WEBHOOK_SECRET=whsec_xxxxxxxxxxxxxxxxxxxxx
```

**The server will crash on startup if `STRIPE_LIVE_SECRET_KEY` is missing or doesn't start with `sk_live_`.**
